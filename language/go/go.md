    ...
    垃圾回收:
        ...
        * 语法垃圾:       
            ...
            * 所接受的挑战:
                ...
                * 三色标记对象漏标:  (写屏障)
                    * 强弱三色不变性
    * 写屏障(为了遵循强弱三色不变性: 黑色无法直接指向白色-强, 黑色只能通过可达灰色路径指向白色):
      * 插入写屏障: 黑色对象 直接引用了白色，强行改写白色为灰色
      * 删除写屏障: 删除灰色-> 白色之前，检测白色的对象引用是否发生变化, 是的话标记为灰色
> 三色标记:
所有的对象都是白色的，从root 出发，引用到的对象从白色-> 灰色，再次扫描灰色队列，白色-> 灰色, 灰色-> 白色；
中间的操锁, 重复完上述，执行写屏障
